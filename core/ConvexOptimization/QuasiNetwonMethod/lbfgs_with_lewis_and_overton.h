#ifndef CORE_CONVEXOPTIMIZATION_QUASINETWONMETHOD_LBFGS_WITH_LEWIS_AND_OVERTON_H
#define CORE_CONVEXOPTIMIZATION_QUASINETWONMETHOD_LBFGS_WITH_LEWIS_AND_OVERTON_H
...
class LimitMemoryCautiousBFGSWithLewis {
  using EigenMatrix = Eigen::Matrix<T, N, N>;
  using EigenVector = Eigen::Matrix<T, N, 1>;

 public:
  LimitMemoryCautiousBFGSWithLewis() = delete;
  ~LimitMemoryCautiousBFGSWithLewis() {}

  LimitMemoryCautiousBFGSWithLewis(const EigenVector& init_pos,
    ...
    LevisAndOvertonConditionParameterCheck(c1_, c2_);
  }
  ...
  void Step(Callable func) {
    ...
    while (!SatisifySearchFinishCondition()) {
      ...
      step_ = init_step_;
      LinearSearchForEigenVectorWithLevisAndOvertonCondition<Callable, T, N>(
...
#endif  // CORE_CONVEXOPTIMIZATION_QUASINETWONMETHOD_LBFGS_WITH_LEWIS_AND_OVERTON_H
