...
class SteepestGradientDescent {
 public:
  ...
  void Step(Function value_function) {
 ...
 private:
  ...
  /// @return
  template <typename Function>
  bool StatisfyGradientDescentFinishCondition(
      const std::array<T, N> &last_iterator, Function value_function) {
 ...
 private:
  float step_;
  float const_parameter_for_amijro_condition_;
